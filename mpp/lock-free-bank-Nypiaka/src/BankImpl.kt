...
class BankImpl(override val numberOfAccounts: Int) : Bank {
    ...
    private inner class TransferOp(val fromIndex: Int, val toIndex: Int, val amount: Long) : Op() {
        ...
        override fun invokeOperation() {
             ...
             * Basically, implementation of this method must perform the logic of the following code "atomically":
             */
            val mn = min(fromIndex, toIndex)
            val mx = max(toIndex, fromIndex)
            val fa = acquire(mn, this)
            val fif = mn == fromIndex
            if (fa != null) {
                if (fif && fa.amount < amount || (!fif && fa.amount + amount > MAX_AMOUNT)) {
                    errorMessage = if (fif) "Underflow" else "Overflow"
                    completed = true
                    release(mn, this)
                } else {
                    val sa = acquire(mx, this)
                    if (sa != null) {
                        val rfa = if (fif) fa else sa
                        val rta = if (fif) sa else fa
                        if (!fif && sa.amount < amount || (fif && sa.amount + amount > MAX_AMOUNT)) {
                            errorMessage = if (!fif) "Underflow" else "Overflow"
                        } else {
                            rfa.newAmount = rfa.amount - amount
                            rta.newAmount = rta.amount + amount
                        }
                        completed = true
                        release(mn, this)
                        release(mx, this)
