...
public class FrontServlet extends HttpServlet {
    ...
    private void process(Route route, HttpServletRequest request, HttpServletResponse response) throws NotFoundException, ServletException, IOException {
        ...
        String[] methodsNames = new String[]{"before", route.getAction(), "after"};
        Map<String, Integer> methodToNumber = Map.of("before", 0, route.getAction(), 1, "after", 2);
        Method[] searchingMethods = new Method[3];
        for (String currentMethodsName : methodsNames) {
            for (Class<?> clazz = pageClass; searchingMethods[methodToNumber.get(currentMethodsName)] == null && clazz != null; clazz = clazz.getSuperclass()) {
                if (searchingMethods[methodToNumber.get(currentMethodsName)] == null) {
                    try {
                        searchingMethods[methodToNumber.get(currentMethodsName)] = clazz.getDeclaredMethod(currentMethodsName, HttpServletRequest.class, Map.class);
        ...
        for (Method method : searchingMethods) {
            if (method == null) {
                throw new NotFoundException();
            }
        }
        ...
        try {
            ...
            throw new ServletException("Can't render template [pageClass=" + pageClass + ", method=" + searchingMethods[1] + "].", e);
        }
    }
